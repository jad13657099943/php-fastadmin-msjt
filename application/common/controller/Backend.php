...
//                $this->error(__('Please login first'), url('index/login', ['url' => $url]));
    ...
    public function qiniuUpload($file)
    {
        ...
        //构建鉴权对象
        $auth = new Auths($config['accessKey'], $config['secrectKey']);
        //生成要上传的token
        ...
        $key = substr(md5($file['tmp_name']), 0, 5) . $this->rand_num(10, 0) . '.' . $ext;
        $filebinary = $file['tmp_name'];

        // 调用 UploadManager 的 put 方法进行文件的上传。
        list($ret, $error) = $upManager->putFile($token, $key, $filebinary);
        $path = $ret['key'];
     ...
     * @param int $type 默认数字+字母,(0数字+大小字母,1数字,2数字+小字母)
     * @return null|string
     * @author zhangxiang <zhxjmdc@gmail.com>
     */
    function rand_num($length, $type = 0)
    {
        $str = null;
        switch ($type) {
            case 0:
