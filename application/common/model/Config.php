...
class Config extends Model
{
     ...
     * @return array
     */
    public static function getGroupList($souce = 'general_config')
    {
        $visible = [
            'all' => ['basic', 'writeoff', 'user','integral', 'merchant', 'sign_basic'],
            'general_config' => ['basic', 'writeoff', 'integral', 'merchant'],
            'sign_signrule' => ['sign_basic'],
        ];

        $groupList = config('site.configgroup');
        foreach ($groupList as $k => &$v) {
            if (!in_array($k, $visible[$souce])) {
                unset($groupList[$k]);
                continue;
            }
            $v = __($v);
     ...
     * 获取系统配置信息数据
     */
    public function getConfigData($where = [])
    {
