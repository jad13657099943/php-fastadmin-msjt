...
class Auth
{
    ...
    public function init($token)
    {
        ...
        if (!$data) {//$this->setError('Account not exist');
    ...
    public function visit($uid)
    {
        ...
        $data_id = $this->visit->where(['create_time' => ['gt', $day_time], 'status' => 2, 'user_id' =>$uid])->value('id');

        if ($data_id) { //修改最后一次访客时间
            $this->visit->save(['create_time' => time()], ['id' => $data_id, 'user_id' => $uid]);
        } else {
            //添加一条记录
            $visit_data = ['status' => 2, 'visit' => 1, 'user_id' => $uid ,'create_time' => time()];
     ...
     * @return boolean
     */
    public function register($password, $mobile, $invite_code, $extend = [])
    {
        $extend = [];
        // 检测邮箱、手机号是否存在
        if ($mobile && User::getByMobile($mobile)) {
        ...
        $data = [
            ...
            'level' => 1,
            'score' => 0,
            'avatar' => '/uploads/20190903/Fpw57TZUNxDBzk3n4OVjkcN65Y1W.png',
        ];
        $params = array_merge($data, [
            'nickname' => $username,
            'salt' => Random::alnum(),
        ...
        if (defined('UC_STATUS') && UC_STATUS) {
            $uc = new \addons\ucenter\library\client\Client();
            $user_id = $uc->uc_user_register($mobile, $password);
            // 如果小于0则说明发生错误
            if ($user_id <= 0) {
                $this->setError($user_id > -4 ? 'mobile is incorrect' : 'Email is incorrect');
                return FALSE;
            } else {
                $params['id'] = $user_id;
            }
        ...
        try {
            ...
            $user_model->score(config('site.reg_integral'), $user->id, '注册送积分', 1);

            // 判断该邀请码是否有用
            if (!empty($invite_code)) {
                $invite_id = $user_model->getField(['invitation_code' => $invite_code], 'id');
                $distributor = $user_model->getField(['id' => $invite_id], 'distributor');
                if ($invite_id && $distributor) {
                    $user_rebate = model('UserRebate');
                    $user_info_add = $user_rebate->add_rebate_data($user->id, $invite_id);
//                    $user_model->invite_num_inc($invite_id);
                    if ($user_info_add != false)
                        $user_model->score(config('site.share'), $invite_id, '邀请好友赠送积分', 1);
                }
            }
            return TRUE;
        } catch (Exception $e) {
    ...
    public function wxRegister($user_info)
    {
        $ip = request()->ip();
        $time = time();
        ...
        $data = [
            ...
            'sex' => empty($user_info['sex']) ? '' : $user_info['sex'],
        ...
        $params = array_merge($data, [
            ...
            'prevtime' => $time,
            'status' => 'normal',
            'superior_id' => $user_info['invitation_code'],
            'wxopenid' => $user_info['openid'],
        ]);
        //账号注册时需要开启事务,避免出现垃圾数据
        Db::startTrans();
        try {
            ...
            if ($user_info['invitation_code']) {
                $superInfo = User::getByInvitationCode($user_info['invitation_code']);
                if ($superInfo) {
                    model('UserRebate')->add_rebate_data($user->id, $superInfo->id);
                }
            }
            Db::commit();
        } catch (Exception $e) {
     ...
     * 用户登录
     *
     * @param string $mobile 账号,用户名、邮箱、手机号
     * @param string $password 密码
     * @return boolean
     */
    public function login($mobile, $password)
    {
        $field = Validate::is($mobile, 'email') ? 'email' : (Validate::regex($mobile, '/^1\d{10}$/') ? 'mobile' : 'username');
        $user = User::get([$field => $mobile]);
        if (!$user) {
            $this->setError('手机号未注册');
            return FALSE;
        }

        if ($user->status != 'normal') {
            $this->setError('账户被锁住'); //Account is locked
            return FALSE;
        }
        if ($user->password != $this->getEncryptPassword($password, $user->salt)) {
            $this->setError('密码不正确'); //Password is incorrect
            return FALSE;
        }
