...
class Mysql extends Driver
{
    ...
    public function set($token, $user_id, $expire = null)
    {
        ...
        $this->handler->insert(['token' => $token, 'user_id' => $user_id, 'createtime' => time(), 'expiretime' => 0]);
        return TRUE;
    }
    ...
    public function get($token)
    {
        ...
        if ($data) {
            if (!$data['expiretime'] || $data['expiretime'] > time()) {
                ...
                $this->clear($data['user_id'] , $this->getEncryptedToken($token));
                return $data;
            } else {
                self::delete($token);
            }
     ...
     * @return  boolean
     */
    public function clear($user_id ,$token = '')
    {   $where['user_id'] = $user_id;
        if($token){
            $where['token'] = ['neq' , $token];
        }
