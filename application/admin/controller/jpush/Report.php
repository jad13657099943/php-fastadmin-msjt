...
class Report extends Backend
{
    ...
    public function recDetail($ids)
    {
        ...
        if (isset($recData['http_code']) && $recData['http_code'] == 200) {
            $data = $recData['body'][0];
           /* $row['android_received']  = $data['android_received'] ?$data['android_received']: 0;
            $row['ios_apns_received'] = $data['ios_apns_received'] ? $data['ios_apns_received']: 0;
            $row['ios_apns_sent']     = $data['ios_apns_sent'] ? $data['ios_apns_sent']:0;
            $row['ios_msg_received']  = $data['ios_msg_received'] ? $data['ios_msg_received']: 0;
            $row['wp_mpns_sent']      = $data['wp_mpns_sent'] ?$data['wp_mpns_sent']: 0;*/
        }
        unset($row->id);
