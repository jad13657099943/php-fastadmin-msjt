    ...
    public function index($kill=0)
    {
        ...
        if ($this->request->isAjax()) {
            ...
            if ($kill== 1){
                ...
                $list = $this->model
                    ->field('goods_id,goods_name,category_id,goods_status,image,deduct_stock_type,goods_price,
                stock_num,sales_actual,goods_sort,status,sales_initial,text_content,marketing_goods_price,vip_price,is_new,upper_num,is_news')
                    ->with(['category', 'freight'])
                    ->where($where)
                    ->where($wheres)
                    ->where('is_new','neq',2)
...
//                        dump($params);dump($spec_many_params);dump($this->request->post(("spec/a")));die;
...
//                        dump($spec_many_params);dump($params['spec_type']);dum($this->request->post(("spec/a")));die;
    ...
    public function edit($ids = NULL)
    {
        ...
        $marketing_id = $this->request->param('marketing_id'); //营销活动id  18
...
//                $params['status'] = implode(',',$params['status']);
                ...
                try {
                    ...
                    $result = $row->allowField(true)->validate(false)->save($params);
    ...
    public function add_marketing($goods_id, $marketing_type, $marketing_id = '')
    {
        ...
        switch ($marketing_type) {
            case 1: //2人团购
                ...
                $param['line_price'] = $goods_info['goods_price'];
            ...
            case 2://限时抢购
                ...
                $infos = model('Limitdiscount')->getLimitDiscountInfoByID($marketing_id, 'start_time , end_time');
                $param = array_merge($goods_info, $infos->getData());
                $param['goods_price'] = $goods_info['marketing_goods_price'];
                $param['line_price'] = $goods_info['line_price'];
                $param['limit_discount_id'] = $marketing_id;
                $param['status'] = $goods_info['goods_status'];
                $param['image'] = $goods_info['image'];
                $param['poster'] = $goods_info['poster'];
                $param['total_stock'] = $goods_info['stock_num'];
                $param['sales'] = $goods_info['sales_actual'];
                $param['upper_num'] = $goods_info['upper_num'];
