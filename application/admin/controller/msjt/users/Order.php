...
class Order extends Backend
{
    ...
    /**
     * Order模型对象
     * @var \app\admin\model\msjt\users\Order
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\msjt\users\Order;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $item) {
                $item->pay_status = $item->status == 1 ? '未支付' : '已支付';
            }
    ...
    public function info($ids = '')
    {
        ...
        $info['goods'] = $this->goods->where('order_no', $info['order_no'])->where('status', 1)->select();
        foreach ($info['goods'] as $k => $good) {
            $info['goods'][$k]['info'] = json_decode($good->info, true);
            $info['goods'][$k]['sku'] = json_decode($good->sku, true);
        }
        ...
        $info['pay_status'] = $info['status'] == 1 ? '未支付' : '已支付';
    ...
    public function setDeliver($ids = '')
    {
        $status = $this->model->where('id', $ids)->update(['status' => 4, 'deliver_time' => time()]);
        if ($status) $this->success('发货成功');
        $this->success('发货失败');
    }
