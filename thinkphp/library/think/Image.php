...
class Image
{
     ...
     * @param int $quality 清晰度 默认100
     * @param int $ext 缩略图拼接字段
     * @return array|string
     */
    public static function setthumb($url, $size = 300, $quality = 100, $ext = "-thumb")
    {    if (strpos($url, $ext) || config('site.image_thumb') == 2)
            return $url;

        $image = self::open(ROOT_PATH . '/public' . $url);
        //获取后缀
        $url = explode('.', $url);
        //组装缩略图的url
        $url = $url[0] . $ext . '.' . $url[1];
        $image->thumb($size, $size)->save(ROOT_PATH . '/public' . $url, null, $quality);
