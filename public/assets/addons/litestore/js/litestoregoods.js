    ...
    GoodsSpec.prototype = {
        ...
        submitAddSpecGroupEvent: function () {
            ...
            _this.$container.on('click', '.btn-addSpecName', function () {
                ...
                // 添加到数据库
                var load = layer.load();
                $.post('litestore/litestoregoods/addSpec', {
        ...
        addSpecItemEvent: function () {
            ...
            _this.$container.on('click', '.btn-addSpecItem', function () {
                ...
                // 添加到数据库
                var load = layer.load();
                $.post('litestore/litestoregoods/addSpecValue', {
         ...
         * 渲染表格html
         */
        /*renderTabelbHtml: function () {
            var $specTabel = this.$container.find('.groupbuy-spec-sku-tabel')
                , $goodsSku = $specTabel.parent();
            // 商品规格为空：隐藏sku容器
            if (data.spec_attr.length === 0) {
                $specTabel.empty();
            ...
            this.buildSpeclist();
            // 渲染table
            $specTabel.html(Template('tpl_groupbuy_spec_table', data));
            // 显示sku容器
            $goodsSku.show();

            //绑定按钮;
            (setting.OutForm).events.plupload($(".groupbuy-spec-sku-tabel"));
            (setting.OutForm).events.faselect($(".groupbuy-spec-sku-tabel"));
        },*/
