define(['jquery', 'bootstrap', 'backend', 'addtabs', 'adminlte', 'form'], function ($, undefined, Backend, undefined, AdminLTE, Form) {
    var Controller = {
        index: function () {
            ...
            setInterval(() => {
                $.ajax({
                    url: '/admin/litestore/litestoreorder/ajax_crontab',
                    success: res => {
                        let msg, url
                        if (res.new_order_count >= 1) {
                            msg = `您有${res.new_order_count}条新订单等待处理`;
                            url = "/admin/litestore/litestoreorder?ref=addtabs";
                        }
                        if (res.new_cut_down_order_count >= 1) {
                            msg = `您有${res.new_cut_down_order_count}条新拼团订单等待处理`;
                            url = "/admin/litestore/litestoreclusterorder?ref=addtabs";
                        }
                        if (res.new_apply_count) {
                            msg = `有${res.new_apply_count}人申请代理商请尽快处理`;
                            url = "/admin/distribution/apply?ref=addtabs";
                        }
                        if (res.new_apply_update_count) {
                            msg = `有${res.new_apply_update_count}个代理商修改个人信息，请注意查看`;
                            url = "/admin/distribution/user?ref=addtabs";
                        }
                        if (res.new_withdraw_coun) {
                            msg = `有${res.new_withdraw_coun}人申请提现请尽快处理`;
                            url = "/admin/distribution/withdraw?ref=addtabs";
                        }
                        if (res.new_refund_count) {
                            msg = `有${res.new_refund_count}条退款订单等待处理`;
                            url = "/admin/litestore/litestoreorderrefund?ref=addtabs";
                        }
                        let dom = document.getElementById('dialog-audio')
                        $('#dialog-audio').attr('autoplay', true)
                        if (res.is_tk == 1) {
                            if ($('.tz-dialog').length < 1) {
                                setTimeout(() => {
                                    $('.wrapper').append(`<div class="tz-dialog"><audio preload="auto" id="dialog-audio" src="/assets/img/noticle.m4a" autoplay  loop /><div class="title flex"><span>提示</span><span class="close">&nbsp;</span></div><div class="tz-msg">${msg}</div><div class="tz-btn-group flex"><a href="${url}" class="btn btn-addtabs" >知道了</a></div></div>`)
                                }, 1000)
                            }
                        } else {
                            $('.tz-dialog .msg').text(msg)
                            $('.tz-dialog btn').attr('href', url)
                        }
                    }
                })
            }, 10000)

            $(document).on('click', '.tz-dialog .close,.tz-dialog .btn', function () {
                $('.tz-dialog').remove()
                $.ajax({
                    url: '/admin/litestore/litestoreorder/close_window',
                    success: res => {
                    }
                })
            })
