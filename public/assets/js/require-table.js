    ...
    var Table = {
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                $(toolbar).on('click', Table.config.qrcodeaa, function () {
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.qrcode_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }
                    Fast.api.open(url, __('自提码'), $(this).data() || {});
                });
                // 导入按钮事件
            ...
            events: {
                ...
                btnGroup: {
                    'click .btn-editones': function (e, value, row, index) {
                        e.stopPropagation();
                        e.preventDefault();
                        var table = $(this).closest('table');
                        var options = table.bootstrapTable('getOptions');
            ...
            formatter: {
                ...
                toggle: function (value, row, index) {
                    ...
                    return "<a href='javascript:;' data-toggle='tooltip' title='" + __('Click to toggle') + "' class='btn-change' data-id='"
                        + row[options.pk] + "' data-params='" + this.field + "=" + (value == yes ? no : yes) + "'><i class='fa fa-toggle-on " + (value == yes ? 'text-' + color : 'fa-flip-horizontal text-gray') + " fa-2x'></i></a>";
                },
                url: function (value, row, index) {
                    return '<div class="input-group input-group-sm" style="width:250px;margin:0 auto;"><input type="text" class="form-control input-sm" value="' + value + '"><span class="input-group-btn input-group-sm"><a href="' + value + '" target="_blank" class="btn btn-default btn-sm"><i class="fa fa-link"></i></a></span></div>';
                },

                btnGroup: function  (value, row, index) {
                    var table = this.table;
                    // 操作配置
                    var options = table ? table.bootstrapTable('getOptions') : {};
                    return '<a href="user/edit/ids/1" class="btn btn-info btn-add" id="modRole" data-toggle="modal" data-target="#editrole" title="修改权限">' + '<span class="glyphicon glyphicon-eye-open">'+ value+'</span></a>';
