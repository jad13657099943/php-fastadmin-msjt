    ...
    public function index()
    {
        ...
        foreach ($crontabList as $crontab) {
            ...
            if ($crontab['maximums'] && $crontab['executes'] > $crontab['maximums']) {
                ...
                //!$cron->isDue(date("YmdHi", $execTime)) ||
                if (date("YmdHi", $execTime) === date("YmdHi", $crontab['executetime']))
                    continue;
                $execute = TRUE;
            }
            ...
            try {
                ...
                if ($crontab['type'] == 'url') {
                    if (substr($crontab['content'], 0, 1) == "/") { echo 1; exit;
                        // 本地项目URL
                        exec('nohup php ' . ROOT_PATH . 'public/index.php ' . $crontab['content'] . ' >> ' . $logDir . date("Y-m-d") . '.log 2>&1 &');
                    } else {
                        // 远程异步调用URL
                        file_get_contents($crontab['content']);
                        //$return = Http::sendAsyncRequest($crontab['content']);
                       // dump($crontab['content']); exit;
                    }
        ...
        return 'Execute completed!'.date("Y-m-d H:i:s").'<br/>';
    }
